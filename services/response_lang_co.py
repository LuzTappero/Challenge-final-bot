...
def generate_response_with_db_langchain(relevant_text, query_text):
    ...
    try:
        if not relevant_text:
            return "Lo siento, no tengo información relevante para responder tu consulta."
        if not query_text:
            raise ValueError("El texto de consulta es requerido.")
        ...
        # Generate the response with the invoke method
        ...
        raise HTTPException(status_code=500, detail="Ocurrió un error inesperado mientras se generaba la respuesta.")
